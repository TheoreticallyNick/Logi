...
=======
    def publish_mqtt(self, JSONpayload, myAWSIoTMQTTClient, test):
>>>>>>> updates
        '''
...
=======
            if not test:
                topic = 'logi2/device/%s'%(self.mqtt.thingName)
            else: 
                topic = 'logi2/test/%s'%(self.mqtt.thingName)
...
<<<<<<< HEAD
=======
    def subscribe_mqtt(self, myAWSIoTMQTTClient):
        '''
        Function: hanldes subscribing to MQTT broker
        '''
        topic = 'logi/command'
            
        logging.info('Subscribed Topic: %s', topic)
        myAWSIoTMQTTClient.subscribe(topic, 0, self.customCallback)
...
=======
            {'alt': 800, 'amb': int(self.mpl.get_tempc()), 'bat': self.bat.get_voltage(), 'ble': 0, 'chg': 0, 'dev': self.mqtt.thingName, 'dts': self.local_time(), 
            'err': self.err, 'ful': self.lvl.get_lvl(), 'gsq': '99, 2, 12', 'lat': '40.12345', 'lon': '-80.12345', 'lsq': '10,0', 'pub': '06:15:10.00',
            'sch': self.schema, 'sol': 2.11, 'sta': 1, 'ver': '1.0, 1.0, 1.0'})
>>>>>>> updates
...
=======
        self.thingName = self.name
        self.mqttClientId = self.name
        self.host = "a28v8anidrrkyj-ats.iot.us-east-1.amazonaws.com"
>>>>>>> updates
